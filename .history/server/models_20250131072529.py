...
class Client(db.Model):
    ...
    appointments = db.relationship('Appointment', backref='client_appointments', lazy=True)
...
class HealthcareProfessional(db.Model):
    ...
    appointments = db.relationship('Appointment', backref='healthcare_professional_appointments', lazy=True)  # Changed backref to avoid conflict
    
    # Linking to the professional_specializations table
    specializations = db.relationship('Specialization', secondary='professional_specializations', lazy='subquery', backref=db.backref('healthcare_professionals', lazy=True))
...
class Appointment(db.Model):
    ...
    client = db.relationship('Client', backref='appointments', lazy=True)  # No conflict with this backref
    healthcare_professional = db.relationship('HealthcareProfessional', backref='appointments_by_healthcare_professional', lazy=True)  # Changed backref to avoid conflict
