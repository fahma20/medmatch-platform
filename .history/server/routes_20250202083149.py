...
class HealthcareProfessionalResource(Resource):
    ...
    def post(self):
        ...
        new_professional = HealthcareProfessional(name=data['name'], status='Active')  # Default status 'Active'
    ...
    def put(self, id):
        ...
        if 'status' in data:
            professional.status = data['status']  # Update status
...
class SpecializationResource(Resource):
    ...
    # Update specialization name if provided
    if 'name' in data:
        specialization.name = data['name']
    
    # Update specialization status if provided
    if 'status' in data:
        specialization.status = data['status']  # Update the status if present in the request body

    db.session.commit()

    # Return the updated specialization
...
class ProfessionalSpecializationUpdateStatusResource(Resource):
    # Update the status of all specializations for a specific healthcare professional
    def put(self):
        try:
            data = request.get_json()
            ...
            # Validate the input
            if not healthcare_professional_id or not new_status:
                return jsonify({"error": "Healthcare professional ID and status are required"}), 400

            # Fetch all the professional specializations for the healthcare professional
            professional_specializations = ProfessionalSpecialization.query.filter_by(
                healthcare_professional_id=healthcare_professional_id
            ).all()

            if not professional_specializations:
                return jsonify({"error": "No specializations found for this healthcare professional"}), 404

            # Update the status of each associated specialization
            for ps in professional_specializations:
                ps.status = new_status

            # Commit the changes to the database
            db.session.commit()

            return jsonify({"message": f"Status for all specializations of healthcare professional {healthcare_professional_id} updated to {new_status}."}), 200
        except Exception as e:
            print(f"Error updating professional specializations: {e}")
            return jsonify({"error": f"Error updating professional specializations: {str(e)}"}), 500
