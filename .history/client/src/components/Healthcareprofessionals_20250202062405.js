...
const HealthcareProfessional = () => {
  ...
  const handleToggleStatus = async (specializationId, status) => {
    try {
        ...
        setProfessionals((prevProfessionals) =>
          prevProfessionals.map((professional) =>
            professional.specializations
              ? {
                  ...professional,
                  specializations: professional.specializations.map((spec) =>
                    spec.id === specializationId ? updatedSpecialization : spec
                  ),
                }
              : professional
          )
          ...
          professionals.map((professional) => (
            <Card key={professional.id} className="mb-3 shadow-sm">
              <Card.Body>
                ...
                {/* Specializations with Status Toggle */}
                    ...
                    {professional.specializations && professional.specializations.length > 0 ? (
                      professional.specializations.map((specialization) => (
                        <ListGroup.Item key={specialization.id}>
                          {specialization.specialization_name} - 
                          <span
                            className={`badge ml-2 ${specialization.status === 'Active' ? 'badge-success' : 'badge-secondary'}`}
                          >
